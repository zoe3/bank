...
## ## campaign + previousで接触総回数を定義
## train <- train %>%
##     dplyr::mutate(total = campaign + previous) 
## test <- test %>%
##     dplyr::mutate(total = campaign + previous)

## ## 接触平均時間を定義
## train <- train %>%
##     dplyr::mutate(ave_time = duration / total) 
## test <- test %>%
##     dplyr::mutate(ave_time = duration / total) 
...
write.table(out, #出力データ
            "C:/bank/submit/submit_1004_1_rpart.csv", #出力先
