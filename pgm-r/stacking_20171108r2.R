...
## 構築データ
train_train<-train_old[row,]

## 検証データ
train_test<- train_old[-row,]

## 比較用に普通に構築
## 決定木作成
str(dplyr::select(train_train, -c(id,age2,month,day,duration2,duration3,balance2,pdays)))
...
##モデルの当てはめ
pred_train_test<- predict(logi_model,
                          newdata=dplyr::select(train_test, -c(id,duration2,duration,age,balance,pdays,job2,job3,education,lastdate,y)),
                          type="response")
